...
def filter_metrics(metrics, select={"ND", "NRMSE", "mean_wQuantileLoss"}):
    return {m: (metrics[m].item() if hasattr(metrics[m], "item") else metrics[m]) 
            for m in select if m in metrics}
...
def time_splitter(data, past_length, future_length):
    ...
    for arr in data:
        ...
        for key, series in arr.items():
            ...
            print(series)
            entry[f"past_{key}"] = arr[:past_length]
            entry[f"future_{key}"] = arr[past_length:past_length + future_length]
        new_dataset.append(entry)
    return new_dataset

def train_test_val_splitter(data,totalLength,ptrain=.6,ptest=.2,pval=.2):
    assert abs(ptrain + ptest + pval - 1.0) < 1e-6, "Splits must sum to 1."
    n = len(data)
    train_end = int(n * ptrain)
    test_end = train_end + int(n * ptest)
